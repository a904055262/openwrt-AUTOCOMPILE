...
parted /dev/${DISK_NAME} mkpart primary ${TARGET_SHARED_FSTYPE} ${START_P3}b "100%"
...
case $TARGET_SHARED_FSTYPE in
	xfs) mkfs.xfs   -f -L "${LB_PRE}SHARED" "/dev/${PT_PRE}3"
	     mount -t xfs     "/dev/${PT_PRE}3" "/mnt/${PT_PRE}3"
	     ;;
      btrfs) mkfs.btrfs -f -L "${LB_PRE}SHARED" "/dev/${PT_PRE}3" 
